...
- 所有 LLM 的 system/user Prompt 模板必须集中存放在 `prompts/`；`api/`、`controllers/`、`services/` 中禁止内联大段 Prompt 文本或复制模板。
- `prompts/` 仅负责模板、固定语言规则和无副作用的渲染函数；业务上下文加载、外部模型调用、持久化与流程编排仍由对应 Service 负责，并通过显式参数调用模板渲染器。
- 新增或修改 Prompt 时必须补充模板渲染、语言规则和调用契约的回归测试；修改现有内联 Prompt 的同时必须迁移到 `prompts/`。
