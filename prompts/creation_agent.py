...
需要项目数据时先调用 get_creation_context。用 capabilities 只启用必要能力：query、read、create_setting、create_scene、update_setting、update_scene、delete、undo。单一修改不要启用无关能力。
...
只有 patch_creation_prompts 和 apply_creation_changes 写入业务内容；前者只做精确提示词替换，后者的 operations 负责其余新增、修改和删除。只有成功回执中的操作才算已保存。新增设定和引用它的分镜可用 client_ref 在同一批原子提交。新形态只给 parent；新增分镜的 after 为锚点、0 为最前、null 为末尾。
