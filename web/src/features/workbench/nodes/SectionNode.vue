...
function saveColor() { store.persistLayout() }
...
<template>
  <section class="workbench-section" :class="{ 'is-selected': selected, 'is-drop-target': data.drop_candidate === true }" :style="backgroundStyle" :aria-label="`${data.title || '画布分区'}，包含 ${memberCount} 个节点`">
    <div v-if="selected" class="workbench-section__toolbar nodrag nowheel" role="toolbar" aria-label="分区操作" @pointerdown.stop @click.stop>
      <button type="button" aria-label="删除分区" title="删除分区（不会删除内部节点）" @click="deleteSection"><Trash2 :size="17" aria-hidden="true" /></button>
      <label class="workbench-section__color-control" title="分区背景颜色"><Palette :size="17" aria-hidden="true" /><input type="color" :value="color" aria-label="修改分区颜色" @pointerdown="beginEdit" @input="updateColor" @change="saveColor"></label>
      <button type="button" aria-label="重新包裹内部节点" title="重新包裹内部节点" :disabled="memberCount === 0" @click="fitToContent"><Minimize2 :size="17" aria-hidden="true" /></button>
      <button type="button" aria-label="分区更多设置" title="更多设置" :class="{ 'is-active': detailsOpen }" @click="detailsOpen = !detailsOpen"><MoreVertical :size="17" aria-hidden="true" /></button>
    </div>
    ...
    <textarea v-if="selected && detailsOpen" class="workbench-section__description nodrag nowheel" :value="description" maxlength="2000" aria-label="分区文字说明" placeholder="添加分区说明…" @focus="beginEdit" @input="updateDescription" @blur="store.persistLayout()" @keydown.stop />
