...
const displayRatio = computed<[number, number]>(() => {
  if (measuredWidth.value > 0 && measuredHeight.value > 0) return [measuredWidth.value, measuredHeight.value]
...
const ratioClass = computed(() => ratioValue.value <= 0.8
  ...
  : ratioValue.value >= 1.25
    ? 'is-landscape'
    : 'is-square')
const mediaStyle = computed(() => ({ aspectRatio: `${displayRatio.value[0]} / ${displayRatio.value[1]}` }))
