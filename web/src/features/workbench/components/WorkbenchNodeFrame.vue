...
<template>
  <article class="workbench-node-frame" :class="{ 'is-selected': selected, 'is-collapsed': collapsed, 'is-ignored': ignored, 'has-marker': markerColor }" :style="markerColor ? { '--workbench-node-marker': markerColor } : undefined" :aria-label="`${data.title || '未命名'}节点`">
    ...
    <template v-if="data.kind === 'video_composer'">
      <Handle id="shot-input" type="target" :position="Position.Left" :connectable="connectable !== false" class="workbench-handle workbench-handle--sequence workbench-handle--composer-shot" aria-label="镜头输入" />
      <Handle id="video-input" type="target" :position="Position.Left" :connectable="connectable !== false" class="workbench-handle workbench-handle--output workbench-handle--composer-video" aria-label="视频输入" />
      <Handle id="watermark-input" type="target" :position="Position.Left" :connectable="connectable !== false" class="workbench-handle workbench-handle--watermark workbench-handle--composer-watermark" aria-label="水印配置" />
    </template>
