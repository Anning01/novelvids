...
export const api = {
  ...
  modelSource: () => request<SingleResponse<{ source: 'official' | 'custom' }>>('/config/team/model-source'),
  setModelSource: (source: 'official' | 'custom') => request<SingleResponse<{ source: 'official' | 'custom' }>>('/config/team/model-source', { method: 'PUT', body: JSON.stringify({ source }) }),
