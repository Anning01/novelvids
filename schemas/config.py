...
class AiModelConfigProperties(BaseModel):
    ...
    thinking: Optional[Literal["enabled", "disabled"]] = Field(
        None,
        description="思考模式：enabled 开启 / disabled 关闭（火山方舟 thinking 参数）",
    )
    max_tokens: Optional[int] = Field(
        None,
        description="单次 LLM 请求最大输出 token 数",
        ge=1,
    )
...
class VideoGenerationCapabilitiesOut(BaseModel):
    ...
    system_audio_asset_scheme: Optional[str] = Field(..., description="系统音色资产引用协议")
    supports_audio_data_uri: bool = Field(..., description="是否支持音频 Base64 Data URI")
    supports_temporary_file_upload: bool = Field(..., description="是否支持临时文件上传")
